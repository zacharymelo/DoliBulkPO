/**
 * Bulk PO — client-side selection persistence, staging area, and
...
(function () {
	...
	var STORAGE_KEY = 'bulkpo_selections';
	...
	function renderStagingArea(store) {
		var emptyEl = document.getElementById('bulkpo-staging-empty');
		var contentEl = document.getElementById('bulkpo-staging-content');
		var countEl = document.getElementById('bulkpo-staging-count');
		var bodyEl = document.getElementById('bulkpo-staging-body');
		var createWrapper = document.getElementById('bulkpo-create-wrapper');
		...
		for (var i = 0; i < ids.length; i++) {
			...
			html += '<td class="right"><input type="number" class="bulkpo-staging-qty flat right" data-product-id="' + item.id + '" value="' + item.qty + '" step="any" min="0.01" style="width:70px;"></td>';
			html += '<td class="center"><a href="#" class="bulkpo-staging-remove" data-product-id="' + item.id + '" title="Remove"><span class="fa fa-trash" style="color:#bc3434;"></span></a></td>';
			html += '</tr>';
		}
	...
	function syncCheckboxesFromStore(store) {
		var checkboxes = document.querySelectorAll('.bulkpo-select');
		for (var i = 0; i < checkboxes.length; i++) {
			var cb = checkboxes[i];
			var pid = cb.getAttribute('data-product-id');
			if (store[pid]) {
				cb.checked = true;
				var qtyInput = document.querySelector('.bulkpo-qty[data-product-id="' + pid + '"]');
				if (qtyInput) {
	...
	function updateSelectAllState() {
		var selectAll = document.getElementById('bulkpo-select-all');
		if (!selectAll) {
			return;
		}
		var checkboxes = document.querySelectorAll('.bulkpo-select');
	...
	function rebuildTableBody(products, vendorId) {
		var cfg = window.bulkpoConfig || {};
		var tbody = document.getElementById('bulkpo-tbody');
		...
		for (var i = 0; i < products.length; i++) {
			...
			html += '<td class="center"><input type="checkbox" class="bulkpo-select" data-product-id="' + p.rowid + '" data-product-ref="' + escapeHtml(p.ref) + '" data-product-label="' + escapeHtml(p.label) + '"></td>';
			...
			html += '<td class="right"><input type="number" class="bulkpo-qty flat right" data-product-id="' + p.rowid + '" value="1" step="any" min="0.01" style="width:70px;"></td>';
			html += '</tr>';
		}
	...
	function rebuildTableHeader(showVendorCols) {
		var thead = document.getElementById('bulkpo-thead');
		if (!thead) {
			return;
		}
		var rows = thead.querySelectorAll('tr');
		// Remove old vendor columns if they exist
		var oldVendorThs = thead.querySelectorAll('.bulkpo-vendor-col');
		...
		if (rows.length >= 2) {
			...
			if (filterTds.length >= 2) {
				...
				td1.className = 'bulkpo-vendor-col';
				filterRow.insertBefore(td1, buyPriceTd);

				var td2 = document.createElement('td');
				td2.className = 'bulkpo-vendor-col';
	...
	function updateFilterInfo(vendorId) {
		var infoDiv = document.getElementById('bulkpo-filter-info');
		if (!infoDiv) {
			return;
		}
		var cfg = window.bulkpoConfig || {};
		if (vendorId > 0) {
			infoDiv.className = 'info bulkpo-filter-info';
	...
	function updateCreateButtonState() {
		var btn = document.getElementById('bulkpo-create-btn');
		if (!btn) {
			return;
		}
		...
		if (socid > 0) {
			...
			btn.title = (window.bulkpoConfig && window.bulkpoConfig.labelSelectVendorFirst) || 'Select a vendor first';
		}
	}

	window.bulkpoUpdateCreateBtn = updateCreateButtonState;
	...
	function setToggleState(isVendor) {
		var btnAll = document.getElementById('bulkpo-show-all');
		var btnVendor = document.getElementById('bulkpo-show-vendor');
		if (!btnAll || !btnVendor) {
			return;
		}
		...
		if (isVendor) {
			...
			btnAll.classList.remove('bulkpo-active');
			btnVendor.classList.add('bulkpo-active');
		} else {
			setButtonClass(btnVendor, 'butAction');
			setButtonClass(btnAll, 'butActionActive');
			btnVendor.classList.remove('bulkpo-active');
			btnAll.classList.add('bulkpo-active');
		}
		btnVendor.title = '';
	}
	...
	window.bulkpoRefreshVendorFilter = function (socid) {
	...
	window.bulkpoResetToAll = function () {
		if (vendorFilterActive) {
			setToggleState(false);
			fetchAndRebuildTable(0);
		}
	...
	function init() {
		...
		document.addEventListener('change', function (e) {
			if (!e.target.classList.contains('bulkpo-select')) {
				return;
			}
			var cb = e.target;
			var pid = cb.getAttribute('data-product-id');
		...
		document.addEventListener('input', function (e) {
			if (!e.target.classList.contains('bulkpo-qty')) {
				return;
			}
		...
		document.addEventListener('click', function (e) {
			// Remove button
			var removeBtn = e.target.closest('.bulkpo-staging-remove');
			if (removeBtn) {
				e.preventDefault();
			...
			// Clear All
			if (e.target.id === 'bulkpo-clear-all' || e.target.closest('#bulkpo-clear-all')) {
				e.preventDefault();
				store = {};
				saveStore(store);
				renderStagingArea(store);
			...
			// Create button
			if (e.target.id === 'bulkpo-create-btn' || e.target.closest('#bulkpo-create-btn')) {
				e.preventDefault();
				var createBtn = document.getElementById('bulkpo-create-btn');
				...
				var form = document.getElementById('bulkpo-form');
				document.getElementById('bulkpo-selected-input').value = JSON.stringify(selections);
				form.querySelector('input[name="action"]').value = 'createorder';
		...
		document.addEventListener('input', function (e) {
			if (!e.target.classList.contains('bulkpo-staging-qty')) {
				return;
			}
			var pid = e.target.getAttribute('data-product-id');
			store = loadStore();
			if (store[pid]) {
				...
				var listQty = document.querySelector('.bulkpo-qty[data-product-id="' + pid + '"]');
				if (listQty) {
					listQty.value = qty;
				}
			}
